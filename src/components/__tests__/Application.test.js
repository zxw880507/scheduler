...
describe("Application", () => {
  ...
  test("loads data, cancels an interview and increases the spots remaining for Monday by 1", async () => {
    ...
    // 3. Click the "Delete" button on the second show appointment.
    ...
    // 7. Wait until the appointment is displayed as empty.
    await waitForElementToBeRemoved(() => getByText(appointment, /deleting/i));

    // 8. Check that the element with the AltText "Add" is displayed.
    expect(getByAltText(appointment, "Add")).toBeInTheDocument();

    // 9. Check that the DayListItem with the text "Monday" also the text "2 spots remaining".
    const day = getAllByTestId(container, "day").find((day) =>
      queryByText(day, "Monday")
    );
  ...
  test("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 2. Wait until the text "Archie Cohen" is displayed and select the second appointment
    await waitForElement(() => getByText(container, "Archie Cohen"));
    const appointment = getAllByTestId(container, "appointment")[1];
    ...
    // 4. Enter the name "Jack Zhao" into the input with the placeholder "Enter Student Name".
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Jack Zhao" },
    });
    // 5. Click to change the interviewer in the list.
    fireEvent.click(getByAltText(appointment, "Sylvia Palmer"));
    ...
    // 8. Wait until the element with the text "Jack Zhao" is displayed.
    await waitForElementToBeRemoved(() => getByText(appointment, /Saving/i));
  ...
  test("shows the save error when failing to save an appointment", async () => {
    ...
    // 7. Wait until the element with the text "ERROR_SAVE" is displayed.
    await waitForElementToBeRemoved(() => getByText(appointment, /Saving/i));
    ...
    // 9. Check the appointment back to creating form.
    expect(getByPlaceholderText(appointment, /enter student name/i)).toBeInTheDocument();

    // 10. Click the "Cancel" button.
    fireEvent.click(getByText(appointment, "Cancel"));

    // 11. Check the appointment back to empty. 
    expect(getByAltText(appointment, "Add")).toBeInTheDocument();

    // 12. Check the DayListItem with the text "Monday" also has the text "1 spot remaining".
    const day = getAllByTestId(container, "day").find((day) =>
      queryByText(day, "Monday")
    );
    expect(getByText(day, /1 spot remaining/i)).toBeInTheDocument();
  ...
  test("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    // 6. Wait until the element with the text "ERROR_DELETE" is displayed.
    ...
    // 9. Check if go back to the second appointment
